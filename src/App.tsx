...
// 2. Firebase Initialization (Safe Mode)
let app: any = null;
let auth: any = null;
let db: any = null;
...
try {
  ...
  // Prevent double init
  // @ts-ignore
  if (!getApp().length) { // Check if already initialized (mock check)
     app = initializeApp(firebaseConfig);
  } else {
     app = getApp(); // Mock
  }
  
  // Re-initialize to be sure if above mock fails in some envs
  app = initializeApp(firebaseConfig, "CurriculumApp"); 
  ...
  // If standard init fails, try accessing default app if exists
  try {
     // @ts-ignore
    app = initializeApp(firebaseConfig); 
    auth = getAuth(app);
    ...
    console.warn("Firebase Init Failed (Offline Mode):", err2);
  }
}
    ...
    <div className="h-screen w-full bg-slate-50 flex items-center justify-center font-sans p-4">
      <div className="max-w-4xl w-full bg-white rounded-2xl shadow-2xl overflow-hidden border border-slate-200 p-8 md:p-12 flex flex-col animate-fadeIn">
        <div className="mb-10 text-center">
          ...
          <h1 className="text-4xl font-black text-slate-800 mb-2 tracking-tight">課程指揮中心 <span className="text-indigo-600">V5.5</span></h1>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-auto bg-slate-100/50 p-4">
          <div className="bg-white rounded shadow-sm border border-slate-300 min-w-[800px]">
             <div className="grid bg-slate-50 border-b sticky top-0 z-10" style={{ gridTemplateColumns: `50px repeat(${config.periodsCount}, 1fr)` }}>
               <div className="p-2 text-center text-[10px] font-bold text-slate-500 border-r">班</div>
               {Array.from({length: config.periodsCount}, (_, i) => i+1).map(p => (
                 <div key={p} className="p-2 text-center border-r text-[10px] font-bold text-slate-500">第 {p} 節</div>
               ))}
             </div>
             <div className="divide-y divide-slate-200">
               {filteredClasses.map(cls => (
                 <div key={cls} className="grid hover:bg-slate-50 transition-colors group" style={{ gridTemplateColumns: `50px repeat(${config.periodsCount}, 1fr)` }}>
                   <div className="p-1 font-bold text-slate-700 border-r flex items-center justify-center bg-slate-100/50 text-xs">{cls}</div>
                   {Array.from({length: config.periodsCount}, (_, i) => i+1).map(p => {
                     ...
                     return (
                       <div 
                         key={p} 
                         className={`border-r last:border-r-0 min-h-[40px] p-0.5 relative cursor-pointer hover:bg-indigo-50/50 transition-colors ${selectedClassInfo?.id === cls ? 'bg-indigo-50/30' : ''}`}
    ...
    <div className="h-screen bg-slate-50 font-sans text-slate-900 flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative">
        ...
        <div className="flex-1 overflow-hidden relative bg-slate-50">
          ...
          {activeTab === 'venue' && <div className="p-10 text-center text-slate-400">地點分配模組 (Venue Module Loaded)</div>}
